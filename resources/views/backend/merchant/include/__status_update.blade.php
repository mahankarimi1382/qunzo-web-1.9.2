    ...
    <div class="site-card-body">
        <div class="row">
            <form action="{{ route('admin.user.status-update', $user->id) }}" method="post">
                ...
                <div class="col-xl-12">
                    <div class="profile-card-single">
                        <h5 class="heading">{{ __('Payment Receive Status') }}</h5>
                        <div class="switch-field">
                            <input type="radio" id="payment_status_enabled" name="payment_status" value="1"
                                @if ($user->payment_status) checked @endif />
                            <label for="payment_status_enabled">{{ __('Active') }}</label>
                            <input type="radio" id="payment_status_disabled" name="payment_status" value="0"
                                @if (!$user->payment_status) checked @endif />
                            <label for="payment_status_disabled">{{ __('Disabled') }}</label>
                        </div>
                    </div>
                </div>
                <div class="col-xl-12">
                    <div class="profile-card-single">
                        ...
                        <div class="switch-field">
                            <input type="radio" id="withdrawSta1" name="withdraw_status" value="1"
                                @if ($user->withdraw_status != null) checked @endif />
                            <label for="withdrawSta1">{{ __('Active') }}</label>
                            <input type="radio" id="withdrawSta2" name="withdraw_status" value="0"
                                @if ($user->withdraw_status == null) checked @endif />
                            <label for="withdrawSta2">{{ __('Disabled') }}</label>
                        </div>
                    </div>
                </div>
                <div class="col-xl-12">
