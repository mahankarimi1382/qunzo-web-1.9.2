                    ...
                    <div class="col">
                        <div class="title-content">
                            ...
                            <a href="" class="title-btn" type="button" data-bs-toggle="modal"
                                data-bs-target="#addNewSchedule"><i data-lucide="plus-circle"></i>{{ __('Add New') }}</a>
                        </div>
                    </div>
                </div>
            </div>
                                    ...
                                    <tbody>
                                        @forelse($schedules as $schedule)
                                            <tr>
                                                ...
                                                <td>{{ $schedule->name }}</td>
                                                <td>
                                                    <strong>{{ $schedule->time }} @php echo $schedule->time > 1 ? __('Hours') : __('Hour') @endphp</strong>
                                                </td>
                                                <td>
                                                    <button class="round-icon-btn primary-btn" type="button" id="edit"
                                                        data-id="{{ $schedule->id }}">
    ...
    <script>
        ...
        $('body').on('click', '#edit', function(event) {
            ...
            $.get('schedule/' + id + '/edit', function(data) {
                ...
                $('#time').val(data.time);
                if (data.time <= 1) {
                    $('#time-level').html('{{ __('Hour') }}');
                } else {
                    $('#time-level').html('{{ __('Hours') }}');
                }
            })
        });
    </script>
