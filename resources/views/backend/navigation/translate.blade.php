        ...
        <div class="d-flex align-items-start back-nav-tab">
            <div class="nav flex-column nav-pills me-3" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                @foreach ($languages as $language)
                    <button class="nav-link {{ $loop->index == 0 ? 'active' : '' }}" id="v-pills-home-tab"
                        data-bs-toggle="pill" data-bs-target="#{{ $language->locale }}" type="button" role="tab"
                        aria-controls="v-pills-home" aria-selected="true">{{ $language->name }}</button>
                @endforeach
            </div>
                                        ...
                                        <form action="{{ route('admin.navigation.translate.now') }}" method="post">
                                            ...
                                            <div class="site-input-groups">
                                                ...
                                                <input type="text" name="name" value="{{ $value['name'] }}"
                                                    class="box-input mb-0" placeholder="Menu Name" required="" />
                                            </div>

                                            <div class="site-input-groups">
                                                <label for=""
                                                    class="box-input-label">{{ __('Megamenu Title:') }}</label>
                                                <input type="text" name="megamenu_name"
                                                    value="{{ $value['megamenu_name'] }}" class="box-input mb-0"
