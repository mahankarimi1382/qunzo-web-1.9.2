...
<form action="{{ route('admin.staff.update', $staff->id) }}" method="post">
    ...
    <div class="site-input-groups">
        ...
        <input type="text" name="name" class="box-input mb-0" value="{{ $staff->name }}" required=""
            id="name" />
    </div>
    <div class="site-input-groups">
        <label for="" class="box-input-label">{{ __('Email:') }}</label>
        <input type="email" name="email" class="box-input mb-0" value="{{ $staff->email }}" required=""
            id="email" />
    </div>
    <div class="site-input-groups">
    ...
    <div class="site-input-groups">
        ...
        <input type="password" name="confirm-password" class="box-input mb-0" />
    </div>
    ...
    <div class="site-input-groups">
        ...
        <select name="role" class="form-select" id="role">
            @foreach ($roles as $role)
                <option @selected($role->name == data_get($staff->roles, '0.name')) value="{{ $role->name }}">
