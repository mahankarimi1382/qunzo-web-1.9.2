        ...
        <div class="site-card">
            ...
            <div class="site-card-body">
                ...
                @foreach ($fields['elements'] as $key => $field)
                            ...
                            <div class="col-sm-8">
                                <div class="form-switch ps-0">
                                    ...
                                    <div class="switch-field same-type m-0">
                                        <input type="radio" id="active-{{ $key }}"
                                            name="{{ $field['name'] }}" value="light"
                                            @if (oldSetting($field['name'], $section) == 'light') checked @endif />
                                        <label for="active-{{ $key }}">{{ __('Light') }}</label>
                                        <input type="radio" id="disable-{{ $key }}"
                                            name="{{ $field['name'] }}" value="dark"
                    ...
                    @elseif($field['name'] == 'language_switcher' || $field['name'] == 'referral_rules_visibility')
                        <div class="site-input-groups row">
                            <div class="col-sm-4 col-label pt-0">{{ __($field['label']) }}</div>
                                ...
                                <div class="form-switch ps-0">
                                    ...
                                    <div class="switch-field same-type m-0">
                                        <input type="radio" id="active-{{ $key }}"
                                            name="{{ $field['name'] }}" value="1"
                                            @if (oldSetting($field['name'], $section)) checked @endif />
                                        <label for="active-{{ $key }}">{{ __('Enabled') }}</label>
                                        <input type="radio" id="disable-{{ $key }}"
                                            name="{{ $field['name'] }}" value="0"
                                            @if (!oldSetting($field['name'], $section)) checked @endif />
