                            ...
                            <div class="col-xl-4 col-lg-4 col-md-3 col-12 col-label">
                                ...
                                @if ($field['name'] == 'site_logo')
                                    <i data-lucide="info" data-bs-toggle="tooltip" title=""
                                        data-bs-original-title="{{ __('Recommended size: 100X42px') }}"></i>
                                @elseif($field['name'] == 'site_favicon')
                                    <i data-lucide="info" data-bs-toggle="tooltip" title=""
                                        data-bs-original-title="{{ __('Recommended size: Square') }}"></i>
                                @elseif($field['name'] == 'login_bg')
                                    <i data-lucide="info" data-bs-toggle="tooltip" title=""
                                        data-bs-original-title="{{ __('Recommended size: 1197X697px') }}"></i>
                                @endif
                            </div>
                            <div class="col-xl-8 col-lg-8 col-md-9 col-12">
                                <div class="wrap-custom-file {{ $errors->has($field['name']) ? 'has-error' : '' }}">
                                    <input type="{{ $field['type'] }}" name="{{ $field['name'] }}"
                                        id="{{ $field['name'] }}" value="{{ setting($field['name'], $section) }}"
                                        accept=".jpeg, .jpg, .png, .webp, .svg" />
                                    <label for="{{ __($field['name']) }}" class="file-ok"
                                        style="background-image: url( {{ asset(setting($field['name'], $section)) }} )">
                                        <img class="upload-icon" src="{{ asset('global/materials/upload.svg') }}"
                                            alt="" />
                                        <span>{{ __('Upload') . ' ' . __($field['label']) }} </span>
                                    </label>
                                    @if (file_exists(public_path(setting($field['name'], $section))))
                                        <div data-name="{{ $field['name'] }}"
                                            data-title="{{ __('Upload') . ' ' . __($field['label']) }}"
                            ...
                            <div class="col-sm-8">
                                <div class="form-switch ps-0">
                                    <div class="switch-field same-type m-0">
                                        <input type="radio" id="active1-global-{{ $key }}"
                                            class="site-currency-type" name="{{ $field['name'] }}" value="fiat"
                                            @checked(oldSetting($field['name'], $section) == 'fiat') />
                                        <label for="active1-{{ $key }}">{{ __('Fiat') }}</label>
                                        <input type="radio" id="disable0-global-{{ $key }}"
                                            name="{{ $field['name'] }}" class="site-currency-type" value="crypto"
                        ...
                        <div class="site-input-groups row">
                            ...
                            <div class="col-sm-8">
                                    ...
                                    <div class="currency-fiat">
                                        <select name="" class="form-select site-currency-fiat" id="">
                                            @if (setting('site_currency_type', 'global') == 'fiat')
                                                <option selected value="{{ oldSetting($field['name'], $section) }}">
                                ...
                                @if ($field['name'] == 'site_timezone')
                                    <select name="{{ $field['name'] }}" class="form-select site-timezone"
                                        id="">
                                        <option selected value="{{ oldSetting($field['name'], $section) }}">
                                            {{ oldSetting($field['name'], $section) }}
                                        </option>
                                ...
                                @if ($field['name'] == 'home_redirect')
                                    <select name="{{ $field['name'] }}" class="form-select" id="">
                                        <option @selected(oldSetting($field['name'], $section) == '/') value="/"> {{ __('Home Page') }}
                                        </option>
                                        @foreach ($pages as $page)
                                            @if ($page->status)
                                                <option @selected(oldSetting($field['name'], $section) == $page->url) value="{{ $page->url }}">
                                                    {{ ucwords($page->title) . ' ' . __('Page') }}
                                                </option>
                                            @endif
                                        @endforeach
                                    </select>
                        ...
                        <div class="site-input-groups row">
                            <label for="" class="col-sm-4 col-label">
                                ...
                                @if (isset($field['info']) && $field['info'])
                                    <i data-lucide="info" data-bs-toggle="tooltip" title=""
                                        data-bs-original-title="{{ $field['info'] }}"></i>
                                @endif
                            </label>
                            <div class="col-sm-8">
                                <div class="input-group joint-input">
                                    ...
                                    @if ($field['name'] == 'site_logo_height' || $field['name'] == 'site_logo_width')
                                        <span class="input-group-text w-25">
                                            <select id="{{ $field['name'] }}" class="form-select">
                                                ...
                                                <option value="auto" @selected(oldSetting($field['name'], $section) == 'auto')>{{ __('Auto') }}
                                                </option>
                                            </select>
                                        </span>
                                    @endif
                                    ...
                                    <input type="{{ $field['type'] }}" name="{{ $field['name'] }}"
                                        ...
                                        value="{{ oldSetting($field['name'], $section) }}" @readonly(oldSetting($field['name'], $section) == 'auto') />
