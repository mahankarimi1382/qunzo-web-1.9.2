@if (setting('passcode_verification', 'permission'))
    @php
        $fields = config('setting.passcode');
    @endphp
                        ...
                        <div class="col-sm-8">
                            <div class="form-switch ps-0">
                                <div class="switch-field same-type m-0">
                                    <input type="radio" id="enable-{{ $field['name'] }}" name="{{ $field['name'] }}"
                                        value="1" @checked(setting($field['name'], 'passcode')) />
                                    <label for="enable-{{ $field['name'] }}">{{ __('Enabled') }}</label>
                                    <input type="radio" id="disable-{{ $field['name'] }}" name="{{ $field['name'] }}"
                                        value="0" @checked(!setting($field['name'], 'passcode')) />
