            ...
            @foreach ($fields['elements'] as $key => $field)
                    ...
                    <div class="site-input-groups row">
                        <div class="col-sm-4 col-label pt-0">{{ __($field['label']) }}
                            <i data-lucide="info" data-bs-toggle="tooltip" title=""
                                data-bs-original-title="{{ __('Do not enable it unless you want the site need to be under Maintenance') }}"></i>
                        </div>
                        <div class="col-sm-8">
                            <div class="form-switch ps-0">
                                ...
                                <div class="switch-field same-type m-0">
                                    <input type="radio" id="{{ $field['name'] }}" name="{{ $field['name'] }}"
                                        value="1" @if (oldSetting($field['name'], $section)) checked @endif />
                                    <label for="{{ $field['name'] }}">{{ __('Enable') }}</label>
                                    <input type="radio" id="disable-{{ $field['name'] }}" name="{{ $field['name'] }}"
                ...
                @elseif($field['type'] == 'textarea')
                    <div class="site-input-groups row">
                        ...
                        <div class="col-sm-8">
                            <textarea name="{{ $field['name'] }}" class="form-textarea  @if ($errors->has($field['name'])) has-error @endif">{{ oldSetting($field['name'], $section) }}</textarea>
                        </div>
                    </div>
                @else
                    <div class="site-input-groups row">
                        <div class="col-sm-4 col-label">{{ __($field['label']) }}
                            @if ($field['name'] == 'secret_key')
                                <i data-lucide="info" data-bs-toggle="tooltip" title=""
                                    data-bs-original-title="{{ __('Remember the Secret Key. Use domain/secret-key to trun back the website live') }}"></i>
                            @endif
                        </div>
                        <div class="col-sm-8">
                            <input type="{{ $field['type'] }}" name="{{ $field['name'] }}"
                                class="box-input @if ($errors->has($field['name'])) has-error @endif"
                                placeholder="{{ __('Label') }}" value="{{ oldSetting($field['name'], $section) }}"
                                required="">
                        </div>
                    </div>
                @endif
