        ...
        @foreach ($notifications as $notification)
            <div class="single-noti">
                <a href="{{ route($notification->for . '.read-notification', $notification->id) }}"
                    ...
                    <div class="content">
                        <div class="main-cont">{{ $notification->notice }}</div>
        ...
        <div class="noti-footer mt-3">
            @if ($totalUnread > 0)
                <a class="noti-btn-1 me-1 w-100"
                    href="{{ route($notifications->first()->for . '.read-notification', 0) }}">
                    {{ __('Mark All as Read') }}
                </a>
            @endif
            <a class="noti-btn-2 ms-1 w-100" href="{{ route($notifications->first()->for . '.notification.all') }}">
