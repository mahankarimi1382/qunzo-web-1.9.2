...
@section('content')
    <div class="main-content">
                    ...
                    <div class="site-card">
                        ...
                        <div class="site-card-body">
                            <form action="{{ route('admin.page.update') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="page_code" value="bill-payment">
                                ...
                                <div class="site-input-groups row mt-4">
                                    <label class="col-sm-12 col-label fw-bold">{{ __('Bill Categories Section') }}</label>
                                </div>
                                <div class="site-input-groups row">
                                    <label for="" class="col-sm-3 col-label">{{ __('Section Title') }}</label>
                                    <div class="col-sm-9">
                                        <input type="text" name="bill_categories_title" class="box-input"
                                            value="{{ $data->bill_categories_title }}">
                                        <small class="text-muted">
                                            {{ __('Use this shortcode to highlight words. Example: [[color_text= Your Text Here ]]') }}
                                        </small>
                                    </div>
                                ...
                                {{-- About Section --}}
                                <div class="site-input-groups row mt-4">
                                    <label class="col-sm-12 col-label fw-bold">{{ __('About Bill Payment Section') }}</label>
                                </div>
                                <div class="site-input-groups row">
                                    <label for="" class="col-sm-3 col-label">{{ __('Section Title') }}</label>
                                    <div class="col-sm-9">
                                ...
                                <div class="site-input-groups row">
                                    <label for="" class="col-sm-3 col-label">{{ __('Section Description') }}</label>
                                    <div class="col-sm-9">
                                        <div class="site-editor fw-normal">
                                            <textarea class="summernote" name="about_description">{{ $data->about_description }}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="site-input-groups row">
                                    <label for="" class="col-sm-3 col-label">{{ __('About Button') }}</label>
                                    <div class="col-sm-9">
                                        <div class="form-row">
                                            <div class="col-xl-4 col-lg-6 col-md-6 col-sm-12 col-12">
                                                <div class="site-input-groups">
                                                    <label for=""
                                                        class="box-input-label">{{ __('Button Label') }}</label>
                                                    <input type="text" name="about_button_label" class="box-input"
                                                        value="{{ $data->about_button_label }}">
                                                </div>
                                            </div>
                                            <div class="col-xl-4 col-lg-6 col-md-6 col-sm-12 col-12">
                                                <div class="site-input-groups">
                                                    <label for=""
                                                        class="box-input-label">{{ __('Button URL') }}</label>
                                                    <input type="text" name="about_button_url" class="box-input"
                                                        value="{{ $data->about_button_url }}">
                                                </div>
                                            </div>
                                            <div class="col-xl-4 col-lg-6 col-md-6 col-sm-12 col-12">
                                                <div class="site-input-groups">
                                                    <label for=""
                                                        class="box-input-label">{{ __('Target') }}</label>
                                                    <select name="about_button_target" class="form-select">
                                                        <option @if ($data->about_button_target == '_self') selected @endif
                                                            value="_self">{{ __('Same Tab') }}</option>
                                                        <option @if ($data->about_button_target == '_blank') selected @endif
                                                            value="_blank">{{ __('Open In New Tab') }}</option>
                                                    </select>
                                                </div>
                                            </div>
                                        ...
                                        <div class="col-xl-9 col-lg-9 col-md-9 col-sm-12">
                                            <div class="wrap-custom-file">
                                                ...
                                                <label for="about_image"
                                                    @if ($data->about_image) class="file-ok"
                                                   style="background-image: url({{ asset($data->about_image) }})" @endif>
                                                    <img class="upload-icon"
        ...
        @php
            $billCategoriesContent = getLandingContents('bill-payment-categories');
            $featuresContent = getLandingContents('bill-payment-features');
            $faqsContent = getLandingContents('bill-payment-faqs');
        @endphp

        {{-- Bill Categories Content Table --}}
        <div class="site-card">
            <div class="site-card-header">
                <h3 class="title">{{ __('Bill Categories Contents') }}</h3>
                <div class="card-header-links">
                    <a href="" class="card-header-link" type="button" data-bs-toggle="modal"
                        data-bs-target="#addNewCategory">{{ __('Add New') }}</a>
                </div>
            </div>
            <div class="site-card-body">
                <div class="site-table table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            @foreach ($billCategoriesContent as $content)
                                <tr>
                                    <td>
                                        @if ($content->icon)
    ...
    <!-- Modal for Add New Bill Category -->
    @include('backend.page.default.section.include.__add_new_bill_payment_category')
    <!-- Modal for Add New Bill Category End -->

    <!-- Modal for Add New Feature -->
    @include('backend.page.default.section.include.__add_new_bill_payment_feature')
    <!-- Modal for Add New Feature End -->

    <!-- Modal for Add New FAQ -->
    @include('backend.page.default.section.include.__add_new_bill_payment_faq')
