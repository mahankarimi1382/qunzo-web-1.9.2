...
@section('content')
    <div class="main-content">
                    ...
                    <div class="site-card">
                        ...
                        <div class="site-card-body">
                            <form action="{{ route('admin.page.update') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="page_code" value="virtual-cards">
                                ...
                                {{-- Confidence Section --}}
                                <div class="site-input-groups row mt-4">
                                    <label class="col-sm-12 col-label fw-bold">{{ __('Confidence Section') }}</label>
                                </div>
                                <div class="site-input-groups row">
                                    <label for="" class="col-sm-3 col-label">{{ __('Section Title') }}</label>
                                    <div class="col-sm-9">
                                        <input type="text" name="confidence_title" class="box-input"
                                            value="{{ $data->confidence_title }}">
                                        <small class="text-muted">
                                            {{ __('Use this shortcode to highlight words. Example: [[color_text= Your Text Here ]]') }}
                                        </small>
                                    </div>
                                ...
                                <div class="site-input-groups row">
                                    ...
                                    <div class="col-sm-9">
                                        <textarea name="confidence_description" class="form-textarea" rows="3">{{ $data->confidence_description }}</textarea>
                                    </div>
                                </div>
                                <div class="site-input-groups row">
                                    <label for=""
                                        class="col-sm-3 col-label">{{ __('Confidence Button') }}</label>
                                    <div class="col-sm-9">
                                        <div class="form-row">
                                            <div class="col-xl-4 col-lg-6 col-md-6 col-sm-12 col-12">
                                                <div class="site-input-groups">
                                                    <label for=""
                                                        class="box-input-label">{{ __('Button Label') }}</label>
                                                    <input type="text" name="confidence_button_label"
                                                        class="box-input" value="{{ $data->confidence_button_label }}">
                                                </div>
                                            </div>
                                            <div class="col-xl-4 col-lg-6 col-md-6 col-sm-12 col-12">
                                                <div class="site-input-groups">
                                                    <label for=""
                                                        class="box-input-label">{{ __('Button URL') }}</label>
                                                    <input type="text" name="confidence_button_url" class="box-input"
                                                        value="{{ $data->confidence_button_url }}">
                                                </div>
                                            </div>
                                            <div class="col-xl-4 col-lg-6 col-md-6 col-sm-12 col-12">
                                                <div class="site-input-groups">
                                                    <label for=""
                                                        class="box-input-label">{{ __('Target') }}</label>
                                                    <select name="confidence_button_target" class="form-select">
                                                        <option @if ($data->confidence_button_target == '_self') selected @endif
                                                            value="_self">{{ __('Same Tab') }}</option>
                                                        <option @if ($data->confidence_button_target == '_blank') selected @endif
                                                            value="_blank">{{ __('Open In New Tab') }}</option>
                                                    </select>
                                                </div>
                                            </div>
                                ...
                                @if ($key == 'en')
                                    <div class="site-input-groups row">
                                        ...
                                        <div class="col-xl-9 col-lg-9 col-md-9 col-sm-12">
                                            <div class="wrap-custom-file">
                                                <input type="file" name="confidence_image" id="confidence_image"
                                                    accept=".gif, .jpg, .png, .svg, .webp" />
                                                <label for="confidence_image"
                                                    @if ($data->confidence_image) class="file-ok"
                                                   style="background-image: url({{ asset($data->confidence_image) }})" @endif>
                                                    <img class="upload-icon"
        ...
        @php
            $howItWorksContent = getLandingContents('virtual-cards-how-it-works');
            $featuresContent = getLandingContents('virtual-cards-features');
            $faqsContent = getLandingContents('virtual-cards-faqs');
        @endphp
    ...
    @include('backend.page.default.section.include.__add_new_virtual_cards_howitworks')
    <!-- Modal for Add New How It Works End -->

    <!-- Modal for Add New Feature -->
    @include('backend.page.default.section.include.__add_new_virtual_cards_feature')
    <!-- Modal for Add New Feature End -->

    <!-- Modal for Add New FAQ -->
    @include('backend.page.default.section.include.__add_new_virtual_cards_faq')
